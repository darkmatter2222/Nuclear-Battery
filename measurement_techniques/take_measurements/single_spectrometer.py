...
def find_aperture(pic_pixels, pic_width: int, pic_height: int) -> object:
    ...
    middle_x_right = int(pic_width / 4) + middle_x
    aperture_brightest = 0
    aperture_x = 0
    for x in range(middle_x_right, pic_width, 1):
    ...
    for x in range(aperture_x, middle_x_right, -1):
        r, g, b = pic_pixels[x, middle_y]
        brightness = r + g + b
...
def draw_ticks_and_frequencies(draw, aperture, spectrum_angle, wavelength_factor):
    ...
    for wl in range(400, 1001, 50):
        ...
        draw.text((x, y0 + aperture_height + 15), str(wl), fill="#fff")
...
def export_diagram(name, normalized_results):
    ...
    for wl in range(400, 1001, 50):
        ...
        wls = str(wl)
        tx = draw.textsize(wls)
        draw.text((x - tx[0] / 2, h + 5 * antialias), wls, fill="#000")
...
def main():
    ...
    #raw_filename = r"C:\Users\ryans\Downloads\9UJdn5t.jpeg"
    ...
    save_image_with_overlay(im, 'test.jpg')
    ...
    export_csv('test.csv', normalized_results)

    # 9. Generate spectrum diagram
    print("generating chart")
    export_diagram('test2.jpg', normalized_results)
